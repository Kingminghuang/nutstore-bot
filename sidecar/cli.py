...
def main() -> int:
    ...
    parser.add_argument(
        "--run-id", type=str, default=str(uuid.uuid4()), help="Run ID (defaults to a new UUID)"
    )
    parser.add_argument(
        "--workspace-path", type=str, default=os.getcwd(), help="Workspace directory path to use. Defaults to the current working directory."
    )
    parser.add_argument("--model-id", type=str, default="gpt-5.4", help="Identifier for the primary model (e.g. gpt-5.4). Used as fallback if --direct-model-id is missing.")
    parser.add_argument(
        "--direct-provider",
        type=str,
    ...
    parser.add_argument(
        "--direct-request-timeout-ms", type=int, default=60000, help="Request timeout in milliseconds for API calls. Defaults to 60000 (60s)."
    )
    parser.add_argument(
        "--max-steps", type=int, default=20, help="Maximum number of turns/steps the agent can execute. Defaults to 20."
    )
    parser.add_argument(
        "--ns-bot-home",
        type=str,
        default=str(Path.home() / ".nsbot"),
        help="Path to the bot's home directory for session and memory storage. Defaults to ~/.nsbot"
    )
    parser.add_argument(
        "--fd-executable", type=str, default="", help="Path to the 'fd' executable tool. Defaults to empty (auto-discovery)."
    )
    parser.add_argument(
        "--rg-executable", type=str, default="", help="Path to the 'rg' (ripgrep) executable tool. Defaults to empty (auto-discovery)."
    )
    parser.add_argument(
        "--tool-os-type", type=str, default="", help="Target OS type for tooling (e.g., win32, darwin). Defaults to current OS."
    )
    parser.add_argument(
        "--session-key", type=str, default="", help="Custom session key for continuing a previous conversation context. Defaults to a workspace-derived key."
    )
    parser.add_argument(
        "--dump-result",
        action="store_true",
    ...
    print(f"[*] Model: {model_disp} (Provider: {config.direct_provider})", file=sys.stderr)
